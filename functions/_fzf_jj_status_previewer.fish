...
# Called as `_fzf_jj_status_previewer revision path`, where path is relative to
...
function _fzf_jj_status_previewer --description 'Helper function for fzf_jj_status to preview the diff of a file'
    command jj diff --color=always --ignore-working-copy --revision $argv[1] -- (_jj_cwd_fileset $argv[2] | string split0)
end
